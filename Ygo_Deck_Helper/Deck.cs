...
namespace Ygo_Deck_Helper
{
    internal class Deck
    {
        ...
        public async static Task<Deck> Create_Deck(List<string> List_Of_Main_Extra_Cards, List<string> list_of_Side_Cards, string Deck_Name)
        {
            ...
            foreach (string Card_Name in List_Of_Main_Extra_Cards)
            {
                KeyValuePair<Card, bool> New_Card_Query = Deck.Query_Single_Card(Card_Name);
                if (New_Card_Query.Value == false)
                {
                    New_Deck.Add_To_Deck(New_Card_Query.Key);
                }
                else
                {
                    KeyValuePair<int, bool> Wikia_Card_Number_Quiery = await YuGiOh_Wiki_Search.Grab_Card_Number(Card_Name);
                    if (Wikia_Card_Number_Quiery.Value == true)
                    {
                        Card New_Card = Deck.Query_Single_Card(Wikia_Card_Number_Quiery.Key).Key;
                        New_Deck.Add_To_Deck(New_Card);
                    }
                    else
                    {
            ...
            foreach (string Card_Name in list_of_Side_Cards)
            {
                KeyValuePair<Card, bool> New_Card_Query = Deck.Query_Single_Card(Card_Name);
                if (New_Card_Query.Value == false)
                {
                    New_Deck.side_deck.Add(New_Card_Query.Key);
                }
                else
                {
                    KeyValuePair<int, bool> Wikia_Card_Number_Quiery = await YuGiOh_Wiki_Search.Grab_Card_Number(Card_Name);
                    ...
                    {
                        Card New_Card = Deck.Query_Single_Card(Wikia_Card_Number_Quiery.Key).Key;
                        New_Deck.side_deck.Add(New_Card);
                    }
                    else
                    {
                        if (New_Deck.Faild_Search_Cards.ContainsKey(Card_Name) == false)
        ...
        public static KeyValuePair<Card, bool> Query_Single_Card(string Card_Name)
        {
            Card New_Card;
            ...
            try
            {
                ...
                DataRow[] Searching_Card_Texts = Sql_Functions.Card_Database.texts.Select(Get_id_Command);
                var id_Object = Searching_Card_Texts[0][0];
                int id = Convert.ToInt32(id_Object);

                Get_Type_Command = "id = '" + id + "'";
                DataRow[] Searching_Card_datas = Sql_Functions.Card_Database.datas.Select(Get_Type_Command);
        ...
        public static KeyValuePair<Card, bool> Query_Single_Card(int Card_Id)
        {
            ...
            try
            {
                ...
                DataRow[] Searching_Card_datas = Sql_Functions.Card_Database.datas.Select(Get_Type_Command);
                DataRow[] Searching_Card_Texts = Sql_Functions.Card_Database.texts.Select(Get_Type_Command);
                var Type_Number_Object = Searching_Card_datas[0][4];
                int Type_Number = Convert.ToInt32(Type_Number_Object);
        ...
        public void Add_To_Deck(Card Card_Added)
        {
            // fix numbers
        ...
        public bool Write_deck(string File_Name)
        {
            ...
            try
            {
                ...
                {
                    Text_Out.WriteLine(Card_To_write.Get_Card_Id());
                }
